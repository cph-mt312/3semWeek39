...
public class Tester {
    ...
    public static void main(String[] args) {
        ...
        // Not necessary due to @OneToOne (cascade...) annotation in Person.java
        // em.persist(a1);
        // em.persist(a2);
        em.persist(p1);
        em.persist(p2);
        ...
        for (Person p : persons) {
            ...
            for (SwimStyle ss : p.styles) {
                System.out.println("---- Styles: " + ss.getStyleName());
            }
        }

        Query q3 = em.createQuery("SELECT new PersonStyleDTO(p.name, p.year, s.styleName) FROM Person p JOIN p.style s");
